...
z3 = Theta2 * a2';
a3 = sigmoid(z3);		% 10 * 50000

H = a3';				% 50000 * 10

y_k = [];				% 50000 * 10
for i = 1:m
    y_k = [y_k; 1:num_labels==y(i)];
end

for k = 1:num_labels
	J += (1 / m) * (-y_k(:,k)' * log(H(:,k)) - (1 - y_k(:,k)') * log(1 - H(:,k)));
end

J += lambda * (1 / (2 * m)) * (sum(sum(Theta1(:, 2:end) .^ 2)) + sum(sum(Theta2(:, 2:end) .^ 2)));
%
